...
func All() []Mutation {
	return []Mutation{
		...
		{
			Name: "card-offers-no-private-number", Package: "./tui/",
			File:        "tui/conncard.go",
			Anchor:      "\tp(\"  Your real number is an allocation the OPERATOR makes. Ask them.\")",
			Replacement: "\tp(\"  Open no more than the per-user cap divided by the number of databases\")\n\tp(\"  your application connects to.\")",
			Test:        "TestCardBudget_ItOffersNoPrivateNumber",
			Fails:       "divided by the number of databases",
			Guarantee:   "that the card does not hand every process the entire shared cap, which is the third of three arithmetic attempts that each looked reasonable in a diff",
		},
		{
			Name: "card-advertises-no-per-source-cap", Package: "./tui/",
			File:        "tui/conncard.go",
			Anchor:      "\tp(\"  Your demand is  processes x databases x connections-per-pool.\")",
		...
		{
			Name: "card-settings-name-their-client", Package: "./tui/",
			File:        "tui/conncard.go",
			Anchor:      "\tp(\"    NOT a HikariCP property, and not something an application sets.\")",
			Replacement: "",
			Test:        "TestCardBudget_TheSettingsNameTheClientTheyBelongTo",
			Fails:       "not separated from HikariCP",
			Guarantee:   "that a tool's own data-source setting is not listed as though it were a library property, which is the mistake made whenever the two share a heading",
		},
		{
