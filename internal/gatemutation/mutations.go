...
func All() []Mutation {
	return []Mutation{
		...
		},
		{
			...
			Anchor:      `p.model.Reset([]tuidecl.Row{{"measure": "unavailable", "value": p.failure}})`,
			Replacement: `p.model.Reset([]tuidecl.Row{{"measure": "capacity", "value": p.failure}})`,
			Test:        "TestPressureView_AnUnreadableViewNamesItselfInsteadOfLookingCalm",
			Fails:       "an unreadable view looked calm",
			Guarantee:   "that an unreadable view names itself rather than looking like a calm front door",
		},
		{
			Name: "pressure-the-pane-shows-the-class", Package: "./tui/",
			File:        "tui/pressure.go",
			Anchor:      `fmt.Sprintf("%d  (%s)", d.Count, d.Class)`,
			Replacement: `fmt.Sprintf("%d", d.Count)`,
			Test:        "TestPressureView_EveryRefusalShowsWhetherItIsUsOrThem",
			Fails:       "refusals lost their class or state",
			Guarantee:   "that every refusal displays capacity or credential instead of a bare count",
		},
		{
			Name: "pressure-no-limit-is-not-a-breach", Package: "./tui/",
			File:        "tui/pressure.go",
			Anchor:      `return fmt.Sprintf("%d  (no limit)", r.Value)`,
			Replacement: `return fmt.Sprintf("%d / 0", r.Value)`,
			Test:        "TestPressureView_NoLimitIsNotShownAsZero",
			Fails:       "unconfigured cap rendered as a zero ceiling",
			Guarantee:   "that an unconfigured cap reads as no limit rather than a breach of zero",
		},
		{
			Name: "pressure-an-unknown-class-is-not-capacity", Package: "./tui/",
			File:        "tui/pressure.go",
			Anchor:      `class := pressure.Credential // an unknown class must not be read as capacity`,
			Replacement: `class := pressure.Capacity // an unknown class must not be read as capacity`,
			Test:        "TestPressureDecode_AnUnknownClassIsNotReadAsCapacity",
			Fails:       "unknown refusal class was guessed as capacity",
			Guarantee:   "that an unknown class is not guessed as capacity, which could trigger a pointless pool resize",
		},
		{
			Name: "pressure-the-decode-keeps-the-remainder", Package: "./tui/",
			File:        "tui/pressure.go",
			Anchor:      `PerUserOmitted:   pressureNumber(m["per_user_omitted"]),`,
			Replacement: `PerUserOmitted:   0,`,
			Test:        "TestPressureDecode_TheViewSurvivesTheWire",
			Fails:       "the wire dropped a timestamp or a remainder",
			Guarantee:   "that the true omitted remainder survives the wire instead of understating pressure",
		},
		{
			Name: "card-a-ceiling-shows-its-figure", Package: "./tui/",
			File: "tui/conncard.go", Anchor: `cardCap(ep.MaxSessionsPerUser)`, Replacement: `""`,
			Test: "TestCardBudget_TheCeilingsCarryTheirScope", Fails: "scope missing its own figure",
			Guarantee: "that each ceiling is printed beside its own scope and figure",
		},
		{
			Name: "card-an-unreported-ceiling-is-not-zero", Package: "./tui/",
			File: "tui/conncard.go", Anchor: `return "not reported"`, Replacement: `return "0"`,
			Test: "TestCardBudget_AnUnreportedCeilingSaysSoRatherThanSayingZero", Fails: "an absent ceiling claimed a number",
			Guarantee: "that an older daemon not reporting a ceiling is not presented as a cap of zero",
		},
		{
			Name: "card-does-no-arithmetic", Package: "./tui/",
			File: "tui/conncard.go", Anchor: `line("  You do not size anything. autodb holds you to these whether or not your")`,
			Replacement: `line("  Divide by the number of databases to find your share.")`,
			Test:        "TestCardBudget_ItDoesNoArithmetic", Fails: "card invented a private share",
			Guarantee: "that the card derives no private share from shared caps",
		},
		{
			Name: "card-advertises-no-per-source-cap", Package: "./tui/",
			File: "tui/conncard.go", Anchor: `line("  For live availability or refusal reasons, open System -> Pressure.")`,
			Replacement: `line("  Per-source concurrency cap: 8")`,
			Test:        "TestCardBudget_NoPerSourceConcurrencyCapIsAdvertised", Fails: "failure throttle posed as capacity",
			Guarantee: "that a credential failure throttle is not sold as a per-source concurrency ceiling",
		},
		{
			Name: "card-tells-nobody-to-configure-a-pool", Package: "./tui/",
			File: "tui/conncard.go", Anchor: `line("  client is configured for them. These ceilings are shared, not yours alone.")`,
			Replacement: `line("  Configure your pool size using these ceilings.")`,
			Test:        "TestCardBudget_ItTellsNobodyToConfigureAPool", Fails: "card handed out pool sizing advice",
			Guarantee: "that the card gives no pool-sizing recipe to a developer",
		},
		{
			Name: "card-shows-no-live-availability", Package: "./tui/",
			File: "tui/conncard.go", Anchor: `line("  For live availability or refusal reasons, open System -> Pressure.")`,
			Replacement: `line("  Available now: 3 backends")`,
			Test:        "TestCardBudget_ItShowsNoLiveAvailability", Fails: "show-once card claimed live availability",
			Guarantee: "that a show-once card does not claim to show instant availability",
		},
		{
			Name: "card-an-unusable-token-gets-no-advice", Package: "./tui/",
			File: "tui/conncard.go", Anchor: `if ep.Configured() {`, Replacement: `if true {`,
			Test: "TestCardBudget_AnUnusableTokenGetsNoBudgetBlock", Fails: "unusable token was buried beneath budget advice",
			Guarantee: "that a token usable nowhere is not buried beneath capacity advice",
		},
		{
			Name: "card-the-connections-own-bound-leads", Package: "./tui/",
			File: "tui/conncard.go", Anchor: `line("  %-22s %-9s %s", "this connection", cardCap(conn.PoolMaxConns), "pooled connections to "+cardDatabase(conn))`,
			Replacement: `line("  %-22s %-9s %s", "sessions per user", cardCap(ep.MaxSessionsPerUser), "you, across databases")`,
			Test:        "TestCardBudget_TheConnectionsOwnBoundLeads", Fails: "connection-owned bound did not lead",
			Guarantee: "that the connection-owned pool bound leads the shared ceilings",
		},
		{
