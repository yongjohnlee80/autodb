...
func (l *Listener) runQuery(ctx context.Context, conn net.Conn, be *pgproto3.Backend,
	...
	// The engine now reports what became of the cut statement (exec.EmitStopped),
	// so the loop no longer infers it. What the emitter can see — a target error
	// that arrives BEFORE the stop — was only ever half the question; a failure
	// arriving after the stop is invisible from here, and that is the half the
	// seam closes.
	...
	case withheld != outputComplete:
		...
		return l.reportOutputWithheld(conn, be, sess, peer, closeReason, withheld, stopped)
...
func (l *Listener) reportOutputWithheld(conn net.Conn, be *pgproto3.Backend,
	...
	lead, effects, outcome := recordedEffects(stopped, status)
...
func recordedEffects(stopped *exec.EmitStopped, status byte) (lead, clause, outcome string) {
	arm := exec.ArmUnresolved
	if stopped != nil {
		arm = stopped.Arm()
	}
	switch arm {
