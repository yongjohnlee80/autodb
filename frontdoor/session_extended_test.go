...
//
// jarvis diagnosed admitSegmentFrame re-admitting a segment that is already
// discarding. The delivery boundary made the existing discard-through-Sync cell
// pass — but a green integration cell is not evidence that re-admission is
// impossible (lector), and a boundary that keeps frames out of reach would hide
// the defect rather than fix it.
//
// This looks at what only re-admission produces: PostgreSQL answers an error
// mid-segment by discarding until Sync and SAYING NOTHING FURTHER. A front door
// that re-admits every subsequent frame emits an ErrorResponse for each one, so
// the count of errors before readiness is the discriminator — one is conformant,
// many is the bug, and the boundary cannot change that count either way.
func TestExtCaps_ADiscardingSegmentIsNotReAdmittedFrameByFrame(t *testing.T) {
	t.Parallel()
	q := okQueries()
	q.extMsgs = []exec.WireMessage{{Kind: "ParseComplete"}}
	_, addr := loopListener(t, q)
	...
	// Well past the cap, so there are many frames AFTER the one that trips it.
	for range maxSegmentMessages + 8 {
		fe.Send(&pgproto3.Describe{ObjectType: 'S', Name: "s"})
	}
	fe.Send(&pgproto3.Sync{})
	if err := fe.Flush(); err != nil {
	...
	errors, sawReady := 0, false
	for !sawReady {
		m, err := fe.Receive()
		if err != nil {
			t.Fatalf("after %d errors and no readiness: %v", errors, err)
		}
		switch m.(type) {
	...
	if errors != 1 {
		t.Fatalf("%d ErrorResponses before readiness, want exactly 1 — PostgreSQL answers an "+
			"error mid-segment by discarding until Sync and saying nothing further, so one "+
			"refusal per over-cap frame means the segment is being RE-ADMITTED while it is "+
			"already discarding", errors)
	}
}
