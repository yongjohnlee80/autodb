...
| `application_name` | **Accepted + audited** (recorded on session + every audit row; also reported back via `ParameterStatus`). Length-capped 256 bytes (over = truncate + notice, audited verbatim). **Not forwarded to the target:** autodb sets no `application_name` on the backend connections it pins (the connection's DSN is used unchanged), so on the target every autodb backend for a connection shows the DSN's value or none — two clients calling themselves `psql` are indistinguishable there. Backend → session attribution is autodb-side (session id on the audit rows). `SET application_name` after startup is refused by the session-state gate. *Stamping a structured per-session value at pin time (target caps it at 63 bytes) is a documented gap, not a decision — 2026-09-03.* |
