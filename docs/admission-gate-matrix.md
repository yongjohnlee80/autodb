...
| `ErrTxAuthorityChanged` | decl session_tx.go:50; raised wire_execute.go:92 and session_engine.go:147 | session, wire | authority | Demotion mid-session; rollback cleanup failed or the transaction's authority ended.  Justification: applicability:§7.5. |
...
| `ErrParamCap` | decl wire_extended_objects.go:45; wire_extended.go:313 | wire ext | intake | Bind > 8192 params.  Justification: applicability:§7.1. |
| `ErrPendingCloseCap` | decl wire_extended_objects.go:59; wire_extended.go:411 | wire ext | resource | A Close whose recovery obligation cannot be tracked — capacity refuses rather than discarding the obligation.  Justification: applicability:§7.1. |
...
| `ErrWireEmitNil` | decl wire_query.go:86; wire_extended.go:481/582/710 | wire ext (engine API) | transport | Programming-error guard on the emit callback.  Justification: applicability:§7.1. |
| `ErrWireFaceLost` | decl wire_query.go:756; wire_query.go:771 (wrap) | wire simple + ext | transport | The pinned connection's raw face is gone; session closes.  Justification: applicability:§7.1. |
| `ErrWireSequenceRefused` | decl wire_query.go:768; wire_query.go:518 | wire simple | transport | A message the loop does not support while an extended segment is open — end it with Sync first.  Justification: applicability:§7.1. |
| `ErrNotExecuted` | decl wire_query.go:186; wire_query.go:346, wire_extended.go:1173 | wire simple + ext | transport | The recorded OUTCOME of a statement PostgreSQL did not run because an earlier one failed — a history status, not a client refusal.  Justification: applicability:§7.1. |
| `ErrDecodedResultTruncated` | decl wire_query.go:745; wire_query.go:729 | wire simple (non-PostgreSQL decoded producer) | transport | A page-bound result on a target that cannot stream unbounded.  Justification: applicability:§7.1. |
