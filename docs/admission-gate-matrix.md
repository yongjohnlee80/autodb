...
| `ErrWireFaceLost` | decl wire_query.go:821; wire_query.go:836 (wrap) | wire simple + ext | transport | The pinned connection's raw face is gone; session closes.  Justification: applicability:§7.1. |
| `ErrWireSequenceRefused` | decl wire_query.go:833; wire_query.go:455, wire_query.go:572, wire_query.go:579, wire_extended.go:115 | wire simple + ext | transport | A message or hidden transaction setup the pinned wire cannot accept while an extended-query segment is open — end it with Sync first.  Justification: applicability:§7.1. |
...
| `ErrDecodedResultTruncated` | decl wire_query.go:809; wire_query.go:794 | wire simple (non-PostgreSQL decoded producer) | transport | A page-bound result on a target that cannot stream unbounded.  Justification: applicability:§7.1. |
