...
|---|---|---|---|---|
| `ErrScriptTooLarge` | decl classify.go:61; raised engine.go:488 (pooled), wire_execute.go:121 (session), wire_query.go:197 and wire_query.go:272 (wire simple), wire_extended.go:173 (wire ext, at Parse) | all four | intake | Before classification everywhere, so the audit record equals what ran. |
