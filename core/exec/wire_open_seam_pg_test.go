...
func openWire(t *testing.T, dsn, appName string) (f *fixture, connID int64, res WireSessionResult, err error) {
	...
	pat, perr := f.svc.CreatePAT(ctx, f.rootTok, fmt.Sprintf("seam-%d", time.Now().UnixNano()), connID, 0, nil, false, nil, testIP)
	if perr != nil {
...
func TestWireOpen_FailsClosedWithoutReporterOrEncodingKeys(t *testing.T) {
	...
	for name, wrap := range map[string]func(golibpg.PinnedConn) any{
		...
		t.Run(name, func(t *testing.T) {
			...
			pat, err := f.svc.CreatePAT(ctx, f.rootTok, fmt.Sprintf("fc-%d", time.Now().UnixNano()), connID, 0, nil, false, nil, testIP)
			if err != nil {
				t.Fatal(err)
			}
