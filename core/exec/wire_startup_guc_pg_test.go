...
func openWireGUCs(t *testing.T, f *fixture, dsn string, gucs map[string]string) (connID int64, res WireSessionResult, err error) {
	...
	pat, perr := f.svc.CreatePAT(ctx, f.rootTok, fmt.Sprintf("guc-%d", time.Now().UnixNano()), connID, 0, nil, false, nil, testIP)
	if perr != nil {
...
func TestWireOpen_StartupGUCsMeetTheSameDenylistAsSET(t *testing.T) {
	...
	pat, perr := f2.svc.CreatePAT(ctx, f2.rootTok, fmt.Sprintf("rguc-%d", time.Now().UnixNano()), connID, 0, nil, false, nil, testIP)
	if perr != nil {
		t.Fatalf("CreatePAT: %v", perr)
	}
