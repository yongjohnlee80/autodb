...
func wireFixture(t *testing.T) (*fixture, *meta.PAT, string, string) {
	...
	newPAT, err := f.svc.CreatePAT(ctx, f.rootTok, "wire", f.connID, 0, nil, false, nil, testIP)
	if err != nil {
...
func TestOpenWireSession_PATNarrowingIsEnforced(t *testing.T) {
	...
	narrowed, err := f.svc.CreatePAT(ctx, f.rootTok, "narrowed", f.connID, 0, []string{"10.1.0.0/16"}, false, nil, testIP)
	if err != nil {
		t.Fatalf("CreatePAT: %v", err)
	}
...
func TestOpenWireSession_TheRemainingRefusals(t *testing.T) {
	...
	t.Run("a user with no grant on the target", func(t *testing.T) {
		...
		erinPAT, err := f.svc.CreatePAT(ctx, erinTok, "erins", f.connID, 0, nil, false, nil, testIP)
		if err != nil {
			t.Fatalf("CreatePAT: %v", err)
		}
