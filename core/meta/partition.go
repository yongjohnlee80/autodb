...
// Monthly RANGE partitioning for the volume tables.
//
...
// the VOLUME class: one row per statement, FK leaves (nothing
...
// answer that a reader can check against the classifier rule.
var volumeTables = []partitionedTable{
	...
	},
	{
		...
		// and user_id 0 records pre-auth events. Partitioning
...
func convertToPartitioned(ctx context.Context, ex migExec, p partitionedTable, now time.Time) error {
	...
	// the whole id story rests on this line.
	if _, err := ex.ExecContext(ctx, fmt.Sprintf(
...
//
// The adoption is the load-bearing half, and it was missing. Postgres
// refuses to create a partition whose range overlaps rows
